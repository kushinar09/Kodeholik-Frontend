...
export default function SkillSection({ skills }) {
    ...
    return (
        <div className="w-full mx-auto space-y-6">
            <div onClick={() => setIsSkillOpen(!isSkillOpen)} className="flex items-center justify-between cursor-pointer">
                ...
                {skills.length > 0 && <Button variant="ghost" size="sm">
                    {!isSkillOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
                </Button>}
            </div>
            <div style={{marginTop: "10px", marginBottom: "10px"}}>
                <motion.div
                    ...
                    <div className="flex flex-row flex-wrap">
                        {skills.map((skill) => (
                            <Badge variant="outline" className="bg-gray-300 py-1.5 mb-4 text-black mr-2">{skill}</Badge>
                        ))}
                    </div>
