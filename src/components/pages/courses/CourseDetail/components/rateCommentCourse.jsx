            ...
            <div className="space-y-6 sm:space-y-8">
              {isAuthenticated ? (
                isEnrolled ? (
                  <Card className="bg-gray-900 border-gray-700">
                    <CardHeader className="pb-2 sm:pb-3">
                      <CardTitle className="text-lg sm:text-xl font-mono text-white">Rate & Comment</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3 sm:space-y-4">
                        <div className="space-y-2">
                          <div className="flex items-center gap-1">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <Button
                                ...
                                variant="ghost"
                                size="sm"
                                className="h-8 w-8 sm:h-9 sm:w-9 p-0 rounded-full hover:bg-transparent"
                          ...
                          {ratingError && <p className="text-red-400 text-xs sm:text-sm">{ratingError}</p>}
                        </div>
                        ...
                        <div className="space-y-2">
                          <Textarea
                            ...
                            className="min-h-[100px] sm:min-h-[120px] bg-gray-800 border-gray-700 text-gray-200 focus:border-gray-500 resize-y text-sm"
                            disabled={submitLoading}
                          />
                          {commentError && <p className="text-red-400 text-xs sm:text-sm">{commentError}</p>}
                        </div>
                        ...
                        <div className="space-y-2">
                          <Button
                            ...
                            className="w-full sm:w-auto bg-primary hover:bg-primary-button-hover text-bg-card text-sm sm:text-base"
                          >
                            {submitLoading ? (
                              <>
                          ...
                          {submitError && <p className="text-red-400 text-xs sm:text-sm">{submitError}</p>}
                          {submitSuccess && <p className="text-green-400 text-xs sm:text-sm">{submitSuccess}</p>}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-gray-900 border-gray-700">
                    <CardContent className="p-4 sm:p-6 text-center">
                      <p className="text-gray-300 mb-4 text-sm sm:text-base">
                        You must enroll in this course to rate and comment.
                      </p>
                      <Button
                        onClick={() => navigate(`/courses/${courseId}`)}
                        className="bg-primary hover:bg-primary-button-hover text-bg-card text-sm sm:text-base"
                      >
                        Enroll Now
                      </Button>
                    </CardContent>
                ...
                <p className="text-gray-300 text-center text-sm sm:text-base">
                  Please log in to rate and comment on this course.
                </p>
              )}

              <div className="space-y-3 sm:space-y-4">
                <h3 className="text-base sm:text-lg font-semibold text-white">Course Reviews</h3>
                      ...
                      {paginatedComments.map((item) => (
                        <Card key={item.id} className="bg-gray-900 border-gray-700 overflow-hidden">
                          <CardContent className="p-3 sm:p-5">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2 sm:mb-3">
                              <div className="flex items-center gap-2 sm:gap-3">
                                ...
                                <div>
                                  ...
                                  <span className="font-medium text-xs sm:text-sm text-gray-200">
                            ...
                            <p className="text-xs sm:text-sm text-gray-300 whitespace-pre-line">{item.comment}</p>
                          </CardContent>
                        </Card>
                      ))}
                    </div>
                    ...
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between mt-4 sm:mt-6">
                        <Button
                          ...
                          className="flex items-center gap-1 sm:gap-2 text-primary hover:bg-primary hover:text-black disabled:opacity-50 rounded-md text-xs sm:text-sm"
                        >
