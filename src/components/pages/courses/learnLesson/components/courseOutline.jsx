...
export default function CourseOutline({ chapters, selectedLesson, activeAccordion, setActiveAccordion, handleLessonSelect }) {
    return (
                ...
                >
                    {chapters.map((chapter, index) => (
                        ...
                        >
                            <AccordionTrigger
                                value={`chapter-${chapter.id}`}
                                className="hover:bg-gray-700/30 px-4 py-4 text-white"
                            >
                            ...
                            <AccordionContent
                                value={`chapter-${chapter.id}`}
                                className="bg-gray-900/30 border-t border-gray-700/50 px-0 py-0"
                            >
                                {chapter.lessons.map((lesson, lessonIndex) => (
                                    <div
                                        key={lesson.id}
                                        className={`flex items-center justify-between p-4 hover:bg-gray-800/50 cursor-pointer transition-colors ${selectedLesson?.id === lesson.id ? "bg-gray-800/80 border-l-2 border-blue-500" : ""
                                            }`}
